...
export default function User() {
  ...
  const removeSkill = (skillToRemove) => {
    setSkillsCheckedData((prevSkills) => prevSkills.filter((skill) => skill.skillId !== skillToRemove.skillId));
  };
  ...
  return (
    <Box>
      ...
      <Grid>
        ...
        <Typography>
          {skillsCheckedData.length} result for
        </Typography>
        </>
      ) : (
        <>
        ...
        {skillsCheckedData?.map((selectedSkills) => {
          return (
            ...
            >
              <IconButton onClick={() => removeSkill(selectedSkills)} size="small">
                <CloseIcon sx={{
                  width:'15px'
                }} />
              </IconButton>
              ...
              >
                {selectedSkills?.skillName}</Typography>
            </Box>
          );
        })}
      </Box>
