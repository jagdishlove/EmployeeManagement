...
export default function MasterData() {
    ...
    useEffect(() => {
      if (officeLocation) {
        ...
        if (officeLocation.address?.countryId) {
          dispatch(getAllState({ parentId: officeLocation.address?.countryId, dataType: 'state' }));
        }
      
        if (officeLocation.address?.stateId) {
          dispatch(GetAllCitys({ parentId: officeLocation.address?.stateId, dataType: 'city' }));
        }
      } if(bandByID){
          setBandFormData({
      ...
      if(clientDetails){
        ...
        if (clientDetails.address?.countryId) {
          dispatch(getAllState({ parentId: clientDetails.address?.countryId, dataType: 'state' }));
        }
      
        if (clientDetails.address?.stateId) {
          dispatch(GetAllCitys({ parentId: clientDetails.address?.stateId, dataType: 'city' }));
        }
      }
      if(onsiteLocation){
        ...
        if (onsiteLocation.address?.countryId) {
          dispatch(getAllState({ parentId: onsiteLocation.address?.countryId, dataType: 'state' }));
        }
      
        if (onsiteLocation.address?.stateId) {
          dispatch(GetAllCitys({ parentId: onsiteLocation.address?.stateId, dataType: 'city' }));
        }

      }
    }, [officeLocation,dispatch,bandByID,holiday,clientDetails,onsiteLocation]);
    ...
    }, [designationExpand, officeLocationExpand,bandExpand,skillExpand,manageHolidayExpand,domineExpand,jobTypeExpand,clientDetailsExpand,clientOfficeLocationExpand]);
    ...
    const handleOfficeChange = async (field, data, value) => {
      ...
      if(field === 'country') {
          ...
          setCountryCode(value?.code)
            if(value?.id){
              await dispatch(getAllState({ parentId:(value?.id || ''),dataType:'state' }));
            }
          ...
          if(value?.id){
          await dispatch(GetAllCitys({ parentId: (value?.id || ''),dataType:'city' }))
          }
      } else if ( field === "city") {
          setOfficeData((prevFormData) => ({
              ...prevFormData,
  ...
  const handleSubmit = async (type, id, status) => {
    ...
    if (Object.keys(validationErrors).length == 0) {
      ...
      try {
        ...
        const payload = {
          locationId :  id ? id : '',
          phoneNumber : clientLocationData.phoneNumber,
          address : {
    ...
    if (Object.keys(validationErrors).length == 0) {
      ...
      try {
        if (type === 'skill') {
          ...
          const payload = {
            locationId : id ? id : '',
            officeAddress : officeData.clinetName,
