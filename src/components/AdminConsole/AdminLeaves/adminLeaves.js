...
} from "../../../redux/actions/AdminConsoleAction/timeSheet/adminTimesheetActionType";
...
const AdminLeaves = () => {
  ...
  const fetchMore = () => {
    const nextPage = pageCounter + 1; // Increment the page counter
    const nextPagePayload = {
      size: 10 * nextPage, // Adjust the size based on the new page counter
      status: leaveStatus,
      date:
        selectedDate !== "All"
          ? dayjs(selectedDate, { format: "YYYY-MM-DD" }).format("YYYY-MM-DD")
    ...
    setPageCounter(nextPage); // Update the page counter
  };
  ...
  const approveRejectLeavesHandler = async (id, status, approverComment) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        ...
        const currentPageSize = 10 * pageCounter;
        await dispatch(
          adminApproveRejectLeavesAction(
            payload,
            { ...getPayload, size: currentPageSize },
  ...
  const handleCheckboxChange = (leaveRequestId) => {
    setSelectedCards((prevSelectedCards) => {
      ...
      // Ensure "Select All" is only checked if all checkboxes are checked
      const allSelected =
        Object.keys(updatedSelectedCards).length ===
          adminLeavesData?.content?.length &&
        Object.values(updatedSelectedCards).every((selected) => selected);
