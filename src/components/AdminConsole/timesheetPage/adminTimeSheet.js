...
function AdminTimeSheet() {
  ...
  }, [states, selectedDate, dispatch,approver]);
  ...
  const handleCheckboxChange = (leaveRequestId) => {
    setSelectedCards((prevSelected) => {
      const updatedSelected = {
        ...prevSelected,
        [leaveRequestId]: !prevSelected[leaveRequestId],
      };
      const allSelected = adminTimeSheetDataStored.length > 0 &&
        adminTimeSheetDataStored.every(entry => updatedSelected[entry.timesheetEntryId]);
  
      // Update the "select all" checkbox only if all checkboxes are selected
  ...
  const handleSelectAll = () => {
    ...
    adminTimeSheetDataStored.forEach(entry => {
    ...
    <Grid>
      ...
      >
        <DialogTitle>
          ...
          /{adminTimeSheetData?.totalElements} Timesheets
        </DialogTitle>
        <DialogContent>
          <Typography>
            Are you sure you want to approve all the selected Timesheets?
          </Typography>
        </DialogContent>
        <DialogActions>
