...
const TimesheetRow = ({
  ...
  console.log("updatedProjectNameList", updatedProjectNameList);
  ...
  const validationForm = () => {
    const newErrors = {};
    console.log("updatedProjectNameListupdatedProjectNameList", selectedValues);
  ...
  const handleSaveData = async () => {
    ...
    try {
      ...
      const timeError = timeValidation(getTimesheetData, {
        fromTime:
          selectedValues.fromTime.length === 5
            ? selectedValues?.fromTime
            : selectedValues?.fromTime?.slice(0, -3),
        toTime:
          selectedValues.toTime.length === 5
            ? selectedValues?.toTime
            : selectedValues?.toTime?.slice(0, -3),
      });
      setErrors(newErrors);
            ...
            >
              ...
              >
                ...
                role === "superAdmin" ? (
                  ...
                  >
                    {!isTimesheetTab ? (
                      managerStatusData?.rating > 0 ? (
                        //  || data.status === "SUBMITTED" || data?.status === "SAVED"
                        <Star
                          onClick={starHandler}
                          approval={approval}
                          data={data}
